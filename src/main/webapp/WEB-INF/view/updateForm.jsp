            ...
            <div class="row no-gutters padds">
					<div class="col-sm-12 col-md-4">
						<figure class=" addBookBox">
									...
									<form:form id="addBook" action="${action}" modelAttribute="book" method="POST">
										...
										<button class="btn btn-lg btn-primary btn-block" type="submit">Aktualizuj książkę</button>
									</form:form>
							</figure>
                </div>
                <div class="col-sm-12 col-md-4">
                    <figure >
                    <p>Tytuł: ${bookU.tytul}</p>
                    <p>Wydawnictwo: ${bookU.wydawnictwo}</p>
                    <p>Dziedzina: ${bookU.dziedzina}</p>
                    <p>Autorzy (kliknij by usunąć):</p>
                        <c:forEach var="autor" items="${bookU.autorzy}">
                            <a href="/cyberlibrary/books/removeAutor?bookId=${bookU.id}&autorId=${autor.id}">
                            ${autor.imie} ${autor.nazwisko}</a>
                        </c:forEach>
                    </figure>
                </div>
				</div>
